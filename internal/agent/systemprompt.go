...
type SystemPromptConfig struct {
	...
	// BrowserLevel controls the browser system prompt section detail ("simple" or "advanced").
	BrowserLevel string
...
func BuildSystemPrompt(cfg SystemPromptConfig) string {
	...
	// 2. ## Tooling
	lines = append(lines, buildToolingSection(cfg.ToolNames, cfg.SandboxEnabled, cfg.ShellDenyGroups, cfg.BrowserLevel)...)
...
func buildToolingSection(toolNames []string, hasSandbox bool, shellDenyGroups map[string]bool, browserLevel string) []string {
	lines := []string{
		"## Tooling",
		"",
	...
	// Browser workflow section — guidance moved from tool Description to save tool-definition tokens.
	if slices.Contains(sortedTools, "browser") {
		lines = append(lines, buildBrowserSection(browserLevel)...)
	}
