...
type PGUserProviderStore struct {
	db      *sql.DB
	encKey  string
}

func NewPGUserProviderStore(db *sql.DB, encryptionKey string) *PGUserProviderStore {
	return &PGUserProviderStore{db: db, encKey: encryptionKey}
}

func (s *PGUserProviderStore) CreateProvider(ctx context.Context, p *store.UserProvider) error {
	now := time.Now()
	p.ID = store.GenNewID()
	p.CreatedAt = now
	p.UpdatedAt = now
...
func (s *PGUserProviderStore) GetProvider(ctx context.Context, id uuid.UUID) (*store.UserProvider, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, user_id, name, display_name, provider_type, api_base, api_key, enabled, settings, created_at, updated_at
		 FROM user_providers WHERE id = $1`, id,
...
func (s *PGUserProviderStore) GetProviderByName(ctx context.Context, userID uuid.UUID, name string) (*store.UserProvider, error) {
	var p store.UserProvider
	err := s.db.QueryRowContext(ctx,
		`SELECT id, user_id, name, display_name, provider_type, api_base, api_key, enabled, settings, created_at, updated_at
		 FROM user_providers WHERE user_id = $1 AND name = $2`, userID, name,
	).Scan(&p.ID, &p.UserID, &p.Name, &p.DisplayName, &p.ProviderType, &p.APIBase, &p.APIKey,
		&p.Enabled, &p.Settings, &p.CreatedAt, &p.UpdatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, sql.ErrNoRows
...
func (s *PGUserProviderStore) ListProviders(ctx context.Context, userID uuid.UUID) ([]store.UserProvider, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, user_id, name, display_name, provider_type, api_base, api_key, enabled, settings, created_at, updated_at
		 FROM user_providers WHERE user_id = $1 ORDER BY name`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *PGUserProviderStore) UpdateProvider(ctx context.Context, id uuid.UUID, updates map[string]any) error {
...
func (s *PGUserProviderStore) DeleteProvider(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM user_providers WHERE id = $1`, id)
	return err
}

func (s *PGUserProviderStore) decryptKey(apiKey, name string) string {
	if s.encKey != "" && apiKey != "" {
